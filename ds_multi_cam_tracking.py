...
# path/camera configuration needed. Unlike the single-cam script, the final
# report is evaluated across ALL cams together (no cam_source filter) since
# cross-camera re-id consistency is exactly what this pipeline exists to
# measure (see chirla_tracking.py's module docstring).
...
def main():
    ...
    if CHIRLA_EVAL_MODE:
        gt_dir, _chirla_source_to_cam = resolve_seq_from_sources(sources, CHIRLA_DATASET_ROOT)
        if not gt_dir:
            sys.stderr.write(
        ...
        # Load only the cameras this run actually tracks - load_ground_truth_seq
        # would pull in EVERY camera_*.json in the seq dir (CHIRLA seqs have up
        # to 7), and evaluate() (called with no cam_source filter below, on
        # purpose, for cross-camera scoring) would then score against cameras
        # this pipeline never produced a single prediction for - manufacturing
        # enormous, meaningless FN instead of measuring the tracker.
        for cam in sorted(set(_chirla_source_to_cam.values())):
            for json_path in glob.glob(os.path.join(gt_dir, f"camera_{cam}_*.json")):
                _chirla_eval.load_ground_truth_json(json_path, cam_source=cam)
        sys.stdout.write(
            f"[chirla-eval] loaded ground truth from {gt_dir} "
